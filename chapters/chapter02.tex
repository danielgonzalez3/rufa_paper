...
This Chapter establishes the context for the RUFA system. \autoref{sec:UFEI} introduces the Urban Forest Ecosystems Institute and its related tools. \autoref{sec:RUFA_Overview} describes RUFA as a software application, its core use cases, and its data sources. \autoref{sec:RUFA_Score} defines the composite score early, then \autoref{sec:Diversity_Metrics} and \autoref{sec:Canopy_Density} provide supporting ecological context for the score components.

\section{Urban Forest Ecosystems Institute} \label{sec:UFEI}

The Urban Forest Ecosystems Institute (UFEI) at Cal Poly addresses the growing need for improved management of California's urban forests. UFEI develops and maintains a suite of public-facing tools and research resources that support tree selection, identification, spatial analysis, and urban forest assessment across the state. These include SelecTree\footnote{\url{https://selectree.calpoly.edu}}, an online tree selection guide covering hundreds of species; the Urban Tree Key\footnote{\url{https://urbantreekey.calpoly.edu}} for field identification; California Big Trees\footnote{\url{https://californiabigtrees.calpoly.edu/}}, which documents champion trees statewide; and the Urban Tree Map\footnote{\url{https://ucfmtm.calpoly.edu}}, a spatial analysis platform for California's inventoried urban trees. UFEI also supports applied research projects that apply remote sensing, machine learning, and ecological modeling to urban forestry questions.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/slo_voronoi.png}
\caption{Interactive map view in RUFA showing tree density by Voronoi cell for San Luis Obispo.}
\label{fig:rufa_map}
\end{figure}
