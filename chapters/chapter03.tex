...
This chapter presents the rationale behind the relational database that underpins RUFA, a system designed to track tree records across geographic and ecological dimensions. A relational database is well suited to the structured data RUFA manages: tree attributes, geographic metrics, and hierarchical relationships between places, ZIP codes, and tracts. The sections that follow examine the MySQL workflow, emphasizing data integrity, normalization principles, materialized views for efficient querying, and indexing strategies that keep queries responsive at scale. Together these choices support scalable, accurate, and high-performance data management for the RUFA project.

At production scale, the workload includes on the order of 7.2 million detector coordinates and tens of millions of inventory-related rows across 702+ places, with interactive dashboard reads targeting well under 200\,ms. Before a score can be served, each tree point must be assigned to the correct administrative polygons, a two-dimensional locality problem for which independent one-dimensional indexes are a poor fit \cite{samet2006}.


\section{Proximity Point Search} \label{sec:ProximitySearch}
...
\paragraph{Quadtree.} Recursively splits 2D space into four quadrants \cite{finkel1974}, subdividing only where data is dense, as shown in \autoref{fig:quadtree}. It is simple and effective for many point-heavy workloads, but balance and depth can vary widely with uneven urban density.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/quad_tree.png}
\caption{Quadtree spatial decomposition}
\label{fig:quadtree}
\end{figure}
...
RUFA uses an R-tree-oriented workflow for polygon layers such as place, ZIP, and tract boundaries. At index-build time, each polygon's MBR is inserted into the tree as a searchable envelope \cite{guttman1984}. \autoref{fig:rtree} shows the resulting structure: leaf rectangles enclose individual geometries, and interior nodes group nearby rectangles into progressively larger envelopes. During lookup, a point query retrieves a very small set of nearest or intersecting envelope candidates, and then runs an exact geometry containment test against the candidate polygons.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/r_tree.png}
\caption{R-tree indexing}
\label{fig:rtree}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \resizebox{\linewidth}{!}{\input{figures/rufa_database_erd}}%
...
Ultimately, although PostgreSQL with PostGIS, MongoDB's geospatial indexes, or specialized GIS databases may outperform MySQL in certain spatial workflows, the RUFA project's current operational environment and alignment with \href{https://selectree.calpoly.edu}{SelecTree}'s architecture underscore MySQL's continued use. Future iterations of RUFA could incorporate or migrate to these alternatives if spatial demands grow significantly, for example by leveraging hybrid architectures that pair MySQL’s relational strengths with specialized spatial extensions in a multi-database ecosystem.

\section{Materialized Views}

A materialized view stores the precomputed result of a query as a physical table, trading storage space and refresh cost for fast reads. RUFA depends on this pattern because its composite scores aggregate millions of tree records across nested geographic boundaries, and recomputing those aggregates on every dashboard request would be far too slow for interactive use. Because MySQL and MariaDB do not provide materialized views natively, RUFA emulates them with regular tables kept current by triggers and scheduled refresh jobs. The full implementation for RUFA Score management is described in \autoref{sec:MVImplementation}.

\section{Indexing Strategies}

Indexing determines whether the queries built on these tables return in milliseconds or seconds. RUFA indexes the columns that drive its most frequent access patterns, including geographic identifiers and the spatial columns discussed in \autoref{sec:ProximitySearch}, so that city- and region-level reads avoid full table scans. Modern relational database management systems maintain these indexes automatically as rows are inserted, updated, or deleted, but at RUFA's scale it remains worthwhile to monitor index health and rebuild fragmented indexes periodically. The effect of indexing on query latency against the full California dataset is quantified in \autoref{sec:IndexingResults}.

\section{Materialized View Implementation for RUFA Score Management} \label{sec:MVImplementation}
...
\subsection{Indexing Results} \label{sec:IndexingResults}
...
\autoref{tab:performance_comparison} presents the average query execution times across different storage and indexing strategies for city-specific queries retrieving tree inventory data.
