...
A well-defined SQL workflow is crucial for maintaining data integrity and ensuring efficient data retrieval \cite{codd1970}. The following subsections will detail the tables used in the RUFA database, the normalization process adopted, and the overall database schema design.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=6.3in]{figures/Rufa_Database.jpeg}
...
\paragraph{MongoDB with Geospatial Indexes.} MongoDB, a NoSQL document database, also offers geospatial indexing that can handle queries for distance-based searches on points, lines, and polygons \cite{cattell2011}. This schema-free approach can be advantageous for storing heterogeneous tree data—especially when the attributes of different species vary significantly—since new fields can be added without redefining a rigid schema.
...
\begin{itemize}
    \item \textbf{Spatial Partitioning}: RUFA scores are partitioned by climate zones and counties to isolate updates
